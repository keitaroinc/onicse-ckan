...
run:
    ...
    ckan -c /etc/ckan/default/production.ini datastore set-permissions | sudo -u postgres psql

Or, if your PostgreSQL server is remote, you can pipe the permissions script
over SSH:

    ckan -c /etc/ckan/default/production.ini datastore set-permissions | ssh dbserver sudo -u postgres psql
